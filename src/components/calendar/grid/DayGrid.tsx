...
const DayGrid: React.FC<DayGridProps> = ({
  ...
  const dayAppointments = useMemo(() => {
    ...
    return appointments.filter((apt) => format(apt.startTime, 'yyyy-MM-dd') === dateKey);
  }, [day, appointments]);
  ...
  const dayOpenings = useMemo(() => {
    ...
    return openings.filter(
      (op) => format(op.openingDate, 'yyyy-MM-dd') === dateKey && !op.isCancelled
    );
  }, [day, openings]);
    ...
    >
      ...
      {isOpeningEditMode && (
        <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 text-xs px-4 py-1.5 font-medium flex items-center gap-2">
          ...
          Mode édition des plages d'ouverture — Cliquez sur une plage pour la modifier ou glissez
      ...
      >
        <div className="flex items-baseline gap-2">
          ...
          <span
            className={cn('text-2xl font-bold', isTodayDate ? 'text-accent' : 'text-foreground')}
          >
            {format(day, 'd')}
          </span>
        ...
        <div className="text-sm text-muted-foreground">{appointmentCount} rendez-vous</div>
      </div>
      ...
      >
        <div className="flex">
          {/* Time Axis */}
          <TimeAxis startHour={startHour} endHour={endHour} slotHeight={slotHeight} />
          ...
          >
            ...
            {/* Now indicator */}
            {isTodayDate && <NowIndicator startHour={startHour} slotHeight={slotHeight} />}
